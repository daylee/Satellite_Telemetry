...
\title{Formal Specification and Runtime Verification of a Communications System for CubeSats}
%\title{A Case Study in Formal Specification and Runtime Verification of a CubeSat Communications System}

\author{Zachary A. Luppen\footnote{Ph.D. Student, Aerospace Engineering, Iowa State University, Student Member.}}
\affil{Iowa State University, zaluppen@iastate.edu}
\author{Dae Young Lee\footnote{Assistant Professor, Aerospace Engineering, Iowa State University.}}
\affil{Iowa State University, daylee@iastate.edu}
\author{Kristin Y. Rozier\footnote{Professor, Aerospace Engineering, Iowa State University.}}
...
Implementing functionality into a CubeSat communications system that monitors for these faults would allow developers to better understand how their system is performing. Developers would then be able to verify what faults, if any, are present in their system. Runtime verification (RV) is designed specifically for this problem. If a specific temporal fault signature can be described using the data available on the system, then an RV engine can be used to monitor for that fault, or send a message to the system bus to properly alert ground crew. One caveat to embedding this software into a communications system is that CubeSat computers tend to have relatively low computing power, meaning most RV engines would be a detriment if embedded on a CubeSat. The Realizable, Responsive, Unobtrusive Unit (R2U2) tool however, requires minimal resources, making it very appealing for embedding RV into a communications system.
...
\begin{enumerate}
  ...
  \item We validate these specifications using experimental evaluation with the R2U2 tool
  \item We highlight and discuss specific patterns noted while developing and revising the RV specifications
  \item Provide lessons learned from validating these specifications
\end{enumerate}
