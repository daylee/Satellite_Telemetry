...
Formal methods have been used in many fields, particularly in the aviation industry and with air traffic control systems to verify performance \cite{GCMTR16,DRR17}. While formal methods have been used in large spacecraft projects, not much literature exists on their inclusion. In addition, %you can't start a sentence with and, so either just delete it or replace the previous period with a comma
...
(Please insert the hardware picture. Have you connected the simulation with the communication hardware? Can you join my webex?)

%Would you like an image of the laptop connected to the radio? I can do that right now! Doing that now
