...
We created, tested and evaluated a model of a CubeSat communications system for the work described in this case study. This model was created using MATLAB/Simulink as well as a software-defined radio (SDR). In the subsections below, we discuss the three major components of this model in-depth.
...
The values we used for our simulation are shown in Table \ref{OrbitalParameters} \textcolor{blue}{Try to rotate the table and minimize the free space}. The simulation time is chosen to be one week in order to provide a decently long time interval with many orbital passes, while also being reasonable for computation time. All values shown in this table, aside from the Earth's radius, the eccentricity and the location of the ground station, are \textcolor{blue}{chosen purely out of experimentation(CySat's orbit parameters?)}. This portion of the model serves to provide us with orbital information -- as well as a visualization -- of a user-defined CubeSat mission rather than building and launching a physical craft. It is important to note that any Earth-orbiting CubeSat orbit could be used for this project, provided that orbit brings it close to the location of a virtual ground station.

With the position and velocity of the CubeSat calculated during this period, we then simulate its interaction with a virtual ground station. We choose the location of the ground station to be 42.0236$^{\circ}$ \textcolor{blue}{lon, lat?} and -93.6528$^{\circ}$ as described in Table \ref{OrbitalDatasets} in the Appendix. This location roughly corresponds to the location of a real ground station located at Iowa State University. While we have chosen this particular location, just about any position on Earth could serve as the location for this virtual station. We also note that the simulation assumes sea-level altitude for the ground station's position.\\

We next use output from the orbital simulation blocks along with the ground station position to determine if line-of-sight (LOS) between the CubeSat and ground station is achieved at each time step. Using the satellite position vector, and ground station position vector in the Earth-centered, Earth-fixed (ECEF) frame along with the ECEF to CubeSat Body quaternion($q_{B/E}$), we calculate the azimuth and elevation of the CubeSat with respect to the ground station. We can then determine whether or not LOS is achieved between the CubeSat and the ground station by monitoring only the calculated elevation angle. A binary representation of when the CubeSat and ground station are in LOS is shown in the third dataplot \textcolor{blue}{(Is it a correct term?)} of Figure \ref{OrbitalDatasets}. The model also keeps track of how many orbits around the Earth are made by the CubeSat, which is shown in the fourth dataplot of Figure \ref{OrbitalDatasets}. At this point, we now have all of the orbital information necessary to proceed with developing simulated communications between the CubeSat and ground station.
...
Formal methods are a set of mathematically rigorous techniques used in the development of software and hardware systems. These techniques have been used in many fields, particularly in the aviation industry and with air traffic control systems to verify performance \cite{GCMTR16,DRR17}. While formal methods have been used in large spacecraft projects, not much literature exists on their inclusion. And CubeSats are a relatively new platform, having only become widely used over the past decade. In the past year, a few research groups have performed case studies applying these techniques to CubeSat projects \cite{Boufaied2020,Gross2020}. These studies improved upon CubeSat performance in strikingly different ways. Boufaied et al. (2020) performed trace-checking to look for specific patterns in data. Gross et al. (2020) developed a decision module that could be used to assess the commands from an unverified controller. These two projects demonstrate the broad applications of formal methods to such projects. For these reasons, applying formal methods to aid in CubeSat design presents an elaborate and as yet hardly touched field of work.
...
The simulated communications system that we have developed runs in $\sim$ 15 min and generates two separate datasets. The first is a set of parameters calculated from the orbital simulation. This data indicates when communication was viable, the position of the craft at each time step, the number of orbits made during the simulation period, etc. Verifying that these datasets adhere to the specifications encoded in MLTL tell us how our ground station is operating and that communications are happening at that proper times. The second dataset is comprised of the telemetry variables transmitted and received using the USRP B210 SDR. This data provides information about the radio system operating on-board the CubeSat and lets us understand the state of the craft. In the following subsections, we compare this data offline, apart from the simulation, against the designed MLTL specifications using the R2U2 tool. In particular, we examine the time necessary to locate faults within these datasets, the amount of memory required by the tool while processing, and other additional statistics detailing the resources required to verify system performance.\\

\subsection{Orbital Dataset Verification}

The orbital dataset is collected throughout the simulation, which covers a weeklong period. The datafile consists of the \textbf{Time} (s), the system's \textbf{Connection} status in boolean representation, the CubeSat's estimated \textbf{Received} data with a 9600 baud rate measured in bits, the system's \textbf{LOS} status in boolean representation,	the CubeSat's \textbf{OrbitNumber} measured as an integer, and the \textbf{Azimuth} and \textbf{Elevation} of the CubeSat measured in degrees. In total, there are 193,025 measurements of these parameters during the simulation.\\
...
\begin{figure}[!ht]
\centering
\includegraphics[width=.7\textwidth]{Fig/OrbitalDataTimes.png}
\caption{Measure of how long it takes for the RV engine to verify progressively larger entries of data. This relationship is linear in time, but the x-axis is logarithmically scaled for better visualization.}
\label{OrbitalDataTimes}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
