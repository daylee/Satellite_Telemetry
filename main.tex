...
Runtime verification (RV) specializes in real-time error detection in these types of temporal, reactive systems, and the R2U2 verification engine proved to fit in the CubeSat's resource constraints. We design specifications to detect and trigger appropriate mitigations for communications system faults. We discuss communications system specification debugging, validation, and variable coverage. Experimental evaluation on simulated datasets demonstrates that running R2U2 within a CubeSat communications system would prevent several unique faults such as atypical position measurements or a voltage supply failure from occurring. We generalize our results and outline our plans for integrating runtime verification into other CubeSat systems as well as verifying an entire telemetry dataset.
