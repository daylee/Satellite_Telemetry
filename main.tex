...
Small satellites are divided into classes based on total mass, such as nanosatellites, picosatellites, and femtosatellites. These low-cost, small spacecraft platforms have cheapened access to space experimentation \cite{Toorian2008}. Cube satellites earn their name due to the modular nature of their sizes. The normal size of a CubeSat is 10cm x 10cm x 10cm, normally referred to as a size of 1U \cite{cubesat101}. CubeSats are not limited to this size however, and can be composed of multiple U units of size. Figure \ref{CubeSatModel} shows a 3U CubeSat. The term "CubeSat" used throughout this case study refers to these small satellites collectively. A typical CubeSat consists of a flight computer, communications system, ADCS, electrical power supply (EPS) and an experimental payload \cite{cubesat101}. A CubeSat is a complex system of systems; the flight computer controls and manages each of the other subsystems throughout a mission. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[!ht]
\centering
\includegraphics[width=.5\textwidth]{Fig/CySat.png}
\caption{A 3d model of a CubeSat of size 3U.}
\label{CubeSatModel}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{MLTL}
% Write with CubeSat developers in mind. They don't know what MLTL is.

The R2U2 tool reasons over specifications written in mission-time linear temporal logic (MLTL) \cite{LVR19,RRS14}. Temporal logic is used to formally describe requirements for a system.

\vspace{\baselineskip}

\textbf{Definition 1.} \textit{(MLTL Syntax) The syntax of an MLTL formula $\phi$ using a set of atomic propositions $\mathcal{AP}$ is recursively defined as the following:}

\begin{equation}
\phi ::= true \; | \; false \; | \; p \; | \; \neg \phi \; | \; \phi_1 \land \phi_2 \; | \; \phi_1 \lor \phi_2 \; | \; \Box_I \phi \; | \; \Diamond_I \phi \; | \; \phi_1 \mathcal{U}_I \phi_2 \; |  \; \phi_1 \mathcal{R}_I \phi_2
\end{equation}

where p $\in$ $\mathcal{AP}$ is an atom and all $\phi$ are atomic propositions. Each symbol stands for the following: $\neg$ = not, $\land$ = logical and, $\lor$ = logical or, $\Box_I$ = globally, $\Diamond_I$ = future, $\mathcal{U}_I$ = until, $\mathcal{R}_I$ = release. \text{I} is an interval [lb,ub], where lb $\leq$, lb, ub $\in$ $\mathbb{N}$.

\vspace{\baselineskip}

\begin{comment}

\textbf{Definition 2.} \textit{(MLTL Semantics) The satisfaction of an MLTL formula $\psi$, over a set of propositions $\mathcal{AP}$, but a computation/trace $\pi$ starting from positions i (denoted as $\pi,i \models \phi$ is recursively defined as the following:}

\begin{itemize}
    \item $\pi, i \models p \in \mathcal{AP}\ \textit{iff}\ p \in \pi[0]$
    \item $\pi, i \models \neg \phi \textit{iff} \pi, i \nvDash \phi$
    \item $\pi, i \models \phi_1 \land \phi_2 \textit{iff} \pi,i \models \phi_1 and \pi,i \models \phi_2$
    \item $\pi,i \models \phi_1 \mathcal{U}_{lb,ub} \phi_2 \textit{iff} | \pi \geq i + lb and there exists j \in [i + lb, i + ub] such that \pi,j \models \phi_2 and for every k < j, k \in [i + lb, i + ub], \pi, k \in \phi_1 $
\end{itemize}
...
This formalism may be unfamiliar to some CubeSat developers, so we provide multiple examples encoding specifications into MLTL in Section \ref{CSRS}. Extracting specifications for cyber-physical systems has proven to be a massive hurdle in practice \cite{Roz16}. Regular English statements pose the difficulty of handling unstated assumptions or implications. These factors pose a risk of misinterpretation when implemented into a real-time system \cite{Roz16}. Encoding these requirements removes this potential layer of abstraction, clearly defining the writer's intent.

For example, a requirement that "the voltage of subsystem X should never exceed 5V during the mission" can be encoded into an MLTL specification in the following manner. First, we define the measured system X voltage as \texttt{sys$\_$voltage} and the 5V maximum to be a bound called \texttt{v$\_$bound}. We write the relation that the system voltage should not exceed this bound with an atomic proposition, shown below:

\begin{equation}
    \phi_1 = \texttt{sys$\_$voltage} \leq \texttt{v$\_$bound}
\end{equation}
...
The simulation time is chosen to be one week in order to provide a non-trivial %"decently long" doesn't feel like a super technical/professional phrase; consider rewording
...
We elicitate specifications for communicating orbital data (Section \ref{ref:orbitspec} and telemetry data (Section \ref{telemspec}) in turn. % We discuss multiple methods for validating our specifications and point out specific patterns that we observed during their development and testing.
%
